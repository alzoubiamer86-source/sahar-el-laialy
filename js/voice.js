...
const Voice = (() => {
  ...
  // ── cleanupAudio — stops ALL incoming audio immediately on ROOM CHANGE ──
  function cleanupAudio() {
    ...
    if (animFrame) { cancelAnimationFrame(animFrame); animFrame = null; }
    if (audioCtx)  { try { audioCtx.close(); } catch {} audioCtx = null; }
    console.log('🔇 Audio cleaned up (room change)');
  }

  // ── Keep audio alive on visibility change (minimize / tab switch) ──
  // Resume AudioContext + resume all audio elements if they paused
  function handleVisibilityChange() {
    if (document.visibilityState === 'visible') {
      // Page came back — resume any paused audio
      ...
      if (localStream && isSpeaker) {
        // Re-enable mic tracks if they got suspended
  ...
  // Allow external update of maxTime after startSpeaking (for rank-based limits)
  ...
  async function onSpeakerJoined(socket, speakerSocketId, nickname) {
    ...
    console.log('🔊 Speaker joined, sending listener_ready to', speakerSocketId.slice(0,8));
  ...
  function onSpeakerLeft(speakerSocketId) {
    ...
    console.log('🔕 Speaker left:', speakerSocketId.slice(0,8));
  }
  ...
  async function onOffer(socket, fromSocketId, offer) {
    ...
    try {
      ...
      console.log('✅ Answer sent to', fromSocketId.slice(0,8));
  ...
  function _createPC(remoteSocketId, socket, isListener) {
    ...
    pc.oniceconnectionstatechange = () => {
      ...
      if (s === 'disconnected') {
        setTimeout(() => { if (pc.iceConnectionState === 'disconnected') _closePeer(remoteSocketId); }, 5000);
      }
    };
  ...
  function _playAudio(socketId, stream) {
    ...
    audio.dataset.voiceSocketId = socketId;
    // Critical: keep playing when tab is hidden / minimized
    audio.setAttribute('x-webkit-airplay', 'deny');
    audio.disableRemotePlayback = true;
    // Prevent browser from suspending audio in background
  ...
  function _startAnalyser(socket) {
    ...
    try {
      // Use 'interactive' latency hint to prevent suspension in background
  ...
  return {
    startSpeaking, stopSpeaking, cleanupAudio, setMaxTime,
    onNewListener, onSpeakerJoined, onSpeakerLeft,
    onOffer, onAnswer, onIce, cleanup,
    handleVisibilityChange,
    isActive: () => isSpeaker,
    getStream: () => localStream,
    _getAudioCtx: () => audioCtx
  };
})();

// ── Keep voice alive on minimize/tab-switch, cleanup only on TRUE unload ──
document.addEventListener('visibilitychange', () => Voice.handleVisibilityChange());

window.addEventListener('beforeunload', () => {
  Voice.cleanup();
});

// pagehide: only cleanup if page is truly being discarded (not cached)
window.addEventListener('pagehide', (e) => {
  if (!e.persisted) Voice.cleanup();
});

// Resume AudioContext on user interaction (browser autoplay policy)
document.addEventListener('click', () => {
  if (Voice._getAudioCtx && Voice._getAudioCtx()?.state === 'suspended') {
    Voice._getAudioCtx().resume().catch(() => {});
  }
}, { once: false });
