...
const Voice = (() => {
  ...
  // ── CRITICAL FIX: cleanupAudio — stops ALL incoming audio immediately ──
  // Called when user leaves a room or refreshes — cuts audio with ZERO delay
  function cleanupAudio() {
    ...
    console.log('🔇 All audio stopped (room change)');
  }
  ...
  function _playAudio(socketId, stream) {
    ...
    // Keep playing even when page is hidden (tab switch / minimize)
  ...
  return {
    startSpeaking, stopSpeaking, cleanupAudio,
...
// ── Cleanup on page unload ONLY (NOT on minimize/tab switch) ──
// We deliberately do NOT listen to visibilitychange here
// so voice keeps working when user minimizes or switches tabs

window.addEventListener('beforeunload', (e) => {
  // Only cleanup on actual page unload, not on visibility hide
  Voice.cleanup();
});

// pagehide = actual navigation away (mobile Safari)
window.addEventListener('pagehide', (e) => {
  if (!e.persisted) {
    // e.persisted=false means page is being discarded (real unload)
    Voice.cleanup();
  }
});
