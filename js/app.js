...
function connectSocket(){
  ...
  s.on('stage_update',({stageUsers})=>{
    ...
    renderStage();
    // Self-healing: the server's stage list is the ground truth. If our
    // local mic UI disagrees with it, force-correct rather than trusting
    // local interval bookkeeping (which has repeatedly proven unreliable
    // across different mic-start code paths — self-request vs admin-grant
    // vs forced revoke vs room switch all used to leave things out of sync).
    const iAmOnStage=state.user && stageUsers.some(u=>u.userId===state.user.id);
    if(state.user && !iAmOnStage && (state.micActive || $('talkBtn')?.classList.contains('active'))){
      state.micActive=false;
      clearInterval(_micTimerInterval);
      $('micTimerBar')&&($('micTimerBar').style.display='none');
      const btn=$('talkBtn');
