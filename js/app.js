...
function connectSocket(){
  ...
  s.on('webrtc_close',({socketId})=>{Voice.onSpeakerLeft(socketId);closeCamPeer(socketId);});
...
function openPermDialog(target){
  ...
  PERMS_LIST.forEach(p=>{const allowed=Array.isArray(maxPerms)?maxPerms.includes(p.key):true;const lbl=el('label','chk-row'+(allowed?'':' perm-disabled'));lbl.innerHTML=`<input type="checkbox" id="perm_${p.key}" ${existing[p.key]?'checked':''} ${allowed?'':'disabled'}> <span>${p.label}</span>`;if(!allowed)lbl.title='خارج نطاق هذه الرتبة';container.appendChild(lbl);});
  $('permModal').style.display='flex';bringToFront($('permModal'));
}
async function savePermissions(){
  if(!state.permTarget)return;
  const perms={};PERMS_LIST.forEach(p=>{perms[p.key]=$('perm_'+p.key)?.checked||false;});
  try{await api('/admin/permissions/'+state.permTarget.userId,'PATCH',{permissions:perms});state.socket?.emit('set_permissions',{targetUserId:state.permTarget.userId,permissions:perms});$('permModal').style.display='none';toast('تم حفظ الصلاحيات ✓','success');loadAccountsPanel();}
  catch(e){state.socket?.emit('set_permissions',{targetUserId:state.permTarget.userId,permissions:perms});$('permModal').style.display='none';toast('تم الإرسال','info');}
}
...
async function loadAccountsPanel(){
  try{
    ...
    filtered.forEach(u=>{
      ...
      tr.innerHTML=`<td><b style="color:${ROLE_COLORS[u.role]||'#000'}">${u.nickname}</b><br><small>${u.username}</small></td><td><span style="color:${ROLE_COLORS[u.role]||'#000'};font-weight:700">${ROLE_LBL[u.role]||u.role}</span></td><td>${u.status==='online'?'🟢':'⚫'} ${u.status||'offline'}</td><td dir="ltr" style="font-size:10px">${u.last_ip||'-'}</td><td>${u.country||'-'}</td><td style="white-space:nowrap"><button class="rm-btn" onclick="changeRole({userId:'${u.id}',nickname:'${u.nickname}',role:'${u.role}'})">رتبة</button> <button class="rm-btn" onclick="openPermDialog({userId:'${u.id}',nickname:'${u.nickname}',role:'${u.role}',permissions:${JSON.stringify(u.permissions||{})}})">صلاحيات</button> <button class="rm-btn rm-btn-red" onclick="confirmDeleteUser('${u.id}','${u.nickname}')">حذف</button></td>`;
      body.appendChild(tr);
    });
