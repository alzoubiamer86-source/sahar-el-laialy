...
function showCtx(e,target){
  ...
  const adm=isAdmin();
  // fix: also allow if user has specific permission granted even if not full admin
  const hasKick=state.user.permissions?.kickout===true;
  const hasMute=state.user.permissions?.muteUsers===true;
  const canAct=(myLevel>targetLevel&&!target.isOwner)||(hasKick&&!target.isOwner&&targetLevel<2);
