...
function proceedAfterLogin(user){
  if(user.role==='owner'){
    // Owner-only fork: choose to enter normally (visible) or invisibly
    // (undercover) right from login, instead of only being able to
    // toggle it after already being in a room.
...
function connectSocket(){
  ...
  s.on('cam_available',({socketId,nickname,avatar})=>{
    // Owner gets both the normal request button AND the silent spy-view
    // button directly in the cam strip — addCamThumb already gates the
    // spy button specifically to isOwner() internally.
...
async function openBanLogDialog(target){
  const reason=await showPrompt('سبب الحظر:','','🚫 حظر بالجهاز/IP');
  if(reason===null)return; // Cancel clicked — abort
  const byIp=await showConfirm('حظر بالـ IP؟ ('+(target.ip||'غير متوفر')+')','🚫 حظر بالـ IP');
  const byMachine=await showConfirm('حظر بالجهاز (fingerprint)؟','🚫 حظر بالجهاز');
...
async function banUser(target){
  const r=await showPrompt('سبب الحظر:','','🚫 حظر '+target.nickname);
  if(r===null)return; // Cancel clicked — abort, don't proceed with the ban
  const d=await showPrompt('المدة بالساعات (فارغ=دائم):','','⏱ مدة الحظر');
  ...
  if(state.camActive){
    ...
    try{
      ...
      try{
        ...
        // Saved camera no longer available (different machine, unplugged) —
        // fall back to the system's current default camera automatically.
...
function switchRmPanel(name){
  ...
  if(name==='advanced'){populateAdvancedSettingsForm();loadSiteCloseAllowedList();}
...
async function loadSiteCloseAllowedList(){
  ...
  try{
    ...
    admins.forEach(u=>{
      ...
      row.innerHTML=`<input type="checkbox" value="${u.id}"> <span>${u.nickname} <small style="color:#888">(${ROLE_LBL[u.role]||u.role})</small></span>`;
