...
function showCtx(e,target){
  ...
  if((hasMutePerm||hasKickPerm||hasBanPerm||hasMicPerm)&&target.userId!==state.user.id){
    ...
    if(hasBanPerm){
      // ★ FIX: Use the new Ban Modal instead of prompts
...
// ★ FIX: Ban User now opens a modal instead of prompts
...
async function doBan(){
  ...
  try {
    if(!isGuest){
      // Registered user — save to DB via REST
      ...
      // Guests have no DB user row — force_ban is the only path
      state.socket?.emit('force_ban',{targetSocketId:target.socketId,reason:r,duration:duration});
    }
