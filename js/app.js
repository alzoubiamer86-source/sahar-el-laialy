...
async function savePermissions(){
  ...
  try{
    ...
    // Also emit via socket for instant live update
    ...
    // API failed (connection issue) — try socket only as fallback
    state.socket?.emit('set_permissions',{targetUserId:state.permTarget.userId,permissions:perms});
    $('permModal').style.display='none';
    toast('تم الإرسال (سيُطبَّق عند الاتصال)','info');
  }
}
...
function saveVoiceSettings(){
  ...
  if(state.currentRoom){
    state.socket?.emit('save_voice_settings',{roomId:state.currentRoom.id,settings:{maxSpeakers:state.roomSettings.maxSpeakers,maxMicTime:state.roomSettings.maxMicTime,freeMic:state.roomSettings.freeMic,rankTimes}});
