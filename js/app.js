...
// RANK_PERMS_DEFAULT: permissions auto-checked when a rank is picked
// (e.g. in "Add Account"). Distinct from RANK_PERMS_MAX which is just the ceiling.
const RANK_PERMS_DEFAULT = {
  user:        [],
  moderator:   ['blockMachine','muteUsers','kickout','sortMicList','clearText','broadcast'],
  super_admin: ['blockMachine','muteUsers','kickout','sortMicList','clearText','broadcast','unban','viewLog'],
  master:      ['blockMachine','muteUsers','kickout','sortMicList','clearText','broadcast','unban','viewLog','manageAccounts','manageMembers','manageAdmins','manageSuperAdmins','roomSettings','adminReports'],
  // owner is never selectable in the Add Account dropdown, so no entry needed here
};
...
function buildAll() {
  ...
  // Bug #4 — auto-populate default permissions when a rank is picked
  $('newAccRole')?.addEventListener('change',applyDefaultPermsForRole);
  applyDefaultPermsForRole();
}
function applyDefaultPermsForRole(){
  const role=$('newAccRole')?.value||'user';
  const defaults=RANK_PERMS_DEFAULT[role]||[];
  PERMS_LIST.forEach(p=>{
    const cb=$('nacp_'+p.key);
    if(cb) cb.checked=defaults.includes(p.key);
  });
}
...
function connectSocket(){
  ...
  s.on('mic_granted',async({by, maxTime=0})=>{
    ...
    if(ok){state.micActive=true;$('talkBtn').classList.add('active');$('muteBtn').classList.remove('active');state.handRaised=false;$('handBtn').classList.remove('active');}
  });
...
function showCtx(e,target){
  ...
  if((adm||hasKickPerm||hasMutePerm)&&target.userId!==state.user.id&&(canAct||hasKickPerm||hasMutePerm)){
    ...
    // Mute — requires muteUsers permission
    if(isAdmin()||state.user.permissions?.muteUsers===true){
    ...
    if(state.user.permissions?.dragToRoom===true||isOwner())add('🚪','نقل إلى غرفة',()=>openDragToRoomDialog(target));
    sep();
    // Kick — requires kickout permission
    if(isAdmin()||state.user.permissions?.kickout===true){
      add('🥾','طرد',()=>kickUser(target),'ctx-danger');
    }
    // Ban — only staff (moderator+)
    if(isAdmin()){
...
async function lockUserMachine(userId,nickname,knownFingerprint){
  ...
  if(!fp){
    fp=prompt('لا يوجد بصمة جهاز معروفة لـ '+nickname+'. أدخلها يدوياً (أو اطلب من المستخدم الدخول أولاً):')||'';
    if(!fp){toast('لا يمكن القفل بدون بصمة جهاز','error');return;}
  } else if(!confirm('قفل جهاز '+nickname+' الحالي؟ لن يتمكن من الدخول من جهاز آخر.')){
    return;
  }
  try{
...
function showFullScreenMsg(title,msg,color='#880000'){
  ...
  d.style.cssText='position:fixed;inset:0;z-index:9999;display:flex;flex-direction:column;align-items:center;justify-content:center;background:rgba(0,0,0,.85);color:#fff;font-family:Cairo,sans-serif;text-align:center;gap:16px;padding:20px';
  d.innerHTML='<div style="font-size:52px">'+title.split(' ')[0]+'</div>'
    ...
    +'<div style="font-size:12px;opacity:.6">جاري تسجيل الخروج...</div>';
